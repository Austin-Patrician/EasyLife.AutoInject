...
public class AutoInjectSourceGenerator : IIncrementalGenerator
{
    ...
    /// </summary>
    private const string GenericAutoInjectAttributeName = "EasyLife.AutoInject.Attributes.AutoInjectAttribute`1";

    /// <summary>
    /// 非泛型AutoInjectAttribute
    /// </summary>
    private const string AutoInjectAttributeName = "EasyLife.AutoInject.Attributes.AutoInjectAttribute";
    ...
    /// .NET8.0以上支持Keyed
    /// </summary>
    private const string AutoInjectKeyedAttributeName = "EasyLife.AutoInject.Attributes.AutoInjectKeyedAttribute`1";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        var nodesAutoInject = context.SyntaxProvider.ForAttributeWithMetadataName(
            ...
            (context, _) => context is ClassDeclarationSyntax cds &&
        ...
        var join = compilationAndTypesInject.Combine(compilationAndTypesInjectG)
            .Combine(compilationAndTypesInjectKeyed);
        ...
        context.RegisterSourceOutput(join,
            ...
            {
                //程序集命名空间
                var @namespace = nodes.Left.Left.Item1.AssemblyName ??
                                 nodes.Left.Right.Item1.AssemblyName ?? nodes.Right.Item1.AssemblyName;
    ...
    private static List<AutoInjectDefine> GetGenericAnnotatedNodesInject(Compilation compilation,
        ...
        foreach (ClassDeclarationSyntax node in nodes.AsEnumerable().Cast<ClassDeclarationSyntax>())
        {
            ...
            foreach (var attr in node.AttributeLists.AsEnumerable())
            {
                ...
                attributeSyntax = attr.Attributes.First(x =>
                    x.Name.ToString().IndexOf(AttributeValueMetadataNameInject, System.StringComparison.Ordinal) == 0);
                    ...
                    string lifeTime = "AddScoped"; //default
                    {
                        if (attributeSyntax.ArgumentList != null)
                        {
                            for (var i = 0; i < attributeSyntax.ArgumentList!.Arguments.Count; i++)
                            {
                                ...
                                {
                                    var name = (expressionSyntax as MemberAccessExpressionSyntax)!.Name.Identifier
                                        .ValueText;
    ...
    /// <param name="compilation"></param>
    /// <param name="nodes"></param>
    private static List<AutoInjectDefine> GetAnnotatedNodesInject(Compilation compilation,
        ImmutableArray<SyntaxNode> nodes)
    {
        if (nodes.Length == 0) return [];
        // 注册的服务
            ...
            foreach (var attr in node.AttributeLists.AsEnumerable())
            {
                ...
                if (attrName?.IndexOf(AttributeValueMetadataNameInject, System.StringComparison.Ordinal) == 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            var eType =
                                (attributeSyntax.ArgumentList!.Arguments[0].Expression as TypeOfExpressionSyntax)!.Type;
    ...
    //获取keyed的Define
    private static List<AutoInjectDefine> GetAnnotatedNodesInjectKeyed(Compilation compilation,
        ImmutableArray<SyntaxNode> nodes)
    {
        if (nodes.Length == 0) return [];
        // 注册的服务
        ...
        foreach (ClassDeclarationSyntax node in nodes.AsEnumerable().Cast<ClassDeclarationSyntax>())
        {
            ...
            foreach (var attr in node.AttributeLists.AsEnumerable())
            {
                var attrName = attr.Attributes
                    .FirstOrDefault(x => x.Name.ToString().Contains(AutoInjectKeyedMetadataNameInject))?.Name
                    .ToString();
                if (attrName is null)
                {
                    continue;
                ...
                attributeSyntax = attr.Attributes.First(x =>
                    x.Name.ToString().IndexOf(AutoInjectKeyedMetadataNameInject, System.StringComparison.Ordinal) == 0);
        ...
        string? rootNamespace)
    {
        ...
        injectDefines.Distinct().ToList().ForEach(define =>
        {
            ...
            if (define.Key != null)
            {
                ...
                {
                    classes.AppendLine(
                        $@"services.{define.LifeTime}<{define.BaseType}, {define.ImplType}>(""{define.Key}"");");
                }
                else
                {
        ...
        envSource = envSource.Replace("$codegen",
        ...
        context.AddSource($"EasyLife.AutoInject.g.cs", SourceText.From(envSource, Encoding.UTF8));
    }
